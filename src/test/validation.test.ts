...
describe("validation", () => {
  ...
  test("rejects xdr with extra keys", () => {
    expect(() =>
      validateAndParseRequest({ xdr: "X", extra: 1 } as any),
    ).toThrow("`xdr` request must not include other parameters");
  });
  ...
  test("rejects missing both", () => {
    expect(() => validateAndParseRequest({})).toThrow(
      "Must pass either `xdr` or `func` and `auth`",
    );
  });

  test("rejects missing func or missing auth", () => {
    expect(() => validateAndParseRequest({ func: "AAAA" } as any)).toThrow(
      "`func` and `auth` are both required",
    );
